...
int s21_is_less(s21_decimal num1, s21_decimal num2) {
  int sign1, sign2, result_less = -1, comparison = 1;
  ...
  if (sign1 && !sign2) {
    result_less = !s21_is_zero_nums(&num1, &num2, result_less);
  } else if (!sign1 && sign2) {
    result_less = 0;
  } else if (sign1 && sign2) {
    for (int i = 3; i >= 0; i--)
      if (num1.bits[i] != num2.bits[i]) {
        comparison = -1;
        result_less =
            s21_is_comparison_decimal(val1, val2, result_less, comparison);
        break;
      } else
        result_less = 0;
  } else
